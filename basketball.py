...
output_file = open("basketball.txt", "w")
...
# Load the webpage
driver.get("https://oddspedia.com/basketball/")
    ...
    for match_div in match_divs:
        ...
        for link in links:
            ...
            if href:
                ...
                if stats_section:
                    h2h_results_primary = stats_section.find("div", class_="h2h-results-secondary")
                    # Process the stats section as needed

                    if h2h_results_primary:
                        h2h_results_primary_items = h2h_results_primary.find("a")

                        data = h2h_results_primary_items.get("title") if h2h_results_primary_items else None
                        if data:
                            update_output_text(f"Teams: {data}\n")
                            hora = soup.find_all("li", class_="h2h-graph__score")
                            score = h2h_results_primary.find_all("a", class_="h2h-results-primary__item")
                            f_scores = h2h_results_primary.find_all("div", class_="h2h-results-secondary__scores")
                            hora = soup.find_all("li", class_="h2h-graph__score")
                            
                            if dates:
                                update_output_text(f"Date: {dates}\n")

                            scores_divs = soup.find_all("div", class_="h2h-results-secondary__scores__overall__inner")

                            for scores_div in scores_divs:
                                first_span = scores_div.find("span", class_="is-winning")
                                second_span = scores_div.find("span", class_="")

                                first_score = int(first_span.get_text(strip=True)) if first_span else 0
                                second_score = int(second_span.get_text(strip=True)) if second_span else 0

                                sum_scores = first_score + second_score

                                update_output_text(f"Score: {sum_scores}\n")
